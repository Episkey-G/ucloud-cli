...
// describeUHostByID returns the poller's describe func, closing over ctx so it
// can build an authed uhost client. Mirrors cmd/uhost.go's describeUHostByID /
// sdescribeUHostByID (unified: the optional commonBase carries region/project/
// zone for the concurrent Sspoll path; the sequential Spoll path passes nil and
// relies on the client's default config region). Returns *uhostsdk.UHostInstanceSet.
func describeUHostByID(ctx *cli.Context) func(uhostID string, commonBase *request.CommonBase) (interface{}, error) {
