...
func bindRegion(req request.Common, flags *pflag.FlagSet) {
	var region string
	flags.StringVar(&region, "region", base.ConfigIns.Region, fmt.Sprintf("Optional. Override default region for this command invocation, see '%s region'", base.BrandNameLower))
...
func bindZone(req request.Common, flags *pflag.FlagSet) {
	var zone string
	flags.StringVar(&zone, "zone", base.ConfigIns.Zone, fmt.Sprintf("Optional. Override default availability zone for this command invocation, see '%s region'", base.BrandNameLower))
...
func bindZoneEmpty(req request.Common, flags *pflag.FlagSet) {
	var zone string
	flags.StringVar(&zone, "zone", "", fmt.Sprintf("Optional. Override default availability zone for this command invocation, see '%s region'", base.BrandNameLower))
	flags.SetFlagValuesFunc("zone", func() []string {
		return getZoneList(req.GetRegion())
	})
	req.SetZoneRef(&zone)
...
func bindProjectID(req request.Common, flags *pflag.FlagSet) {
	var project string
	flags.StringVar(&project, "project-id", base.ConfigIns.ProjectID, fmt.Sprintf("Optional. Override default project-id for this command invocation, see '%s project list'", base.BrandNameLower))
