...
func NewCmdULBCreate(out io.Writer) *cobra.Command {
	...
	bindRegion(req, flags)
	bindProjectID(req, flags)
	req.VPCId = flags.String("vpc-id", "", fmt.Sprintf("Optional. Resource ID of VPC which the ULB to create belong to. See '%s vpc list'", base.BrandNameLower))
	req.SubnetId = flags.String("subnet-id", "", fmt.Sprintf("Optional. Resource ID of subnet. This flag will be discarded when you are creating an outter mode ULB. See '%s subnet list'", base.BrandNameLower))
